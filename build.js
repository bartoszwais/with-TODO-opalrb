...
/* Generated by Opal 0.8.0 */
Opal.modules["application-todo"] = function(Opal) {
  Opal.dynamic_require_severity = "error";
  var self = Opal.top, $scope = Opal, nil = Opal.nil, $breaker = Opal.breaker, $slice = Opal.slice, $klass = Opal.klass;

  Opal.add_stubs(['$find', '$new', '$add_class', '$html', '$on', '$prevent', '$puts', '$append', '$html=', '$value', '$remove', '$value=']);
  return (function($base, $super) {
    function $TODO(){};
    ...
    return (def.$initialize = function() {
      var $a, $b, TMP_1, $c, TMP_2, self = this, list = nil, form = nil, text_box = nil, button_send_list_to_email = nil;

      list = $scope.get('Document').$find("#list");
      form = $scope.get('Document').$find("#form");
      text_box = form.$find("input");
      button_send_list_to_email = $scope.get('Element').$new("button");
      button_send_list_to_email.$add_class("send_email");
      button_send_list_to_email.$html("send as email");
      ($a = ($b = button_send_list_to_email).$on, $a.$$p = (TMP_1 = function(event){var self = TMP_1.$$s || this;
if (event == null) event = nil;
      event.$prevent();
        return self.$puts("TODO: send list entries in email");}, TMP_1.$$s = self, TMP_1), $a).call($b, "click");
      list.$append(button_send_list_to_email);
      return ($a = ($c = form).$on, $a.$$p = (TMP_2 = function(event){var self = TMP_2.$$s || this, $a, $b, TMP_3, $c, item = nil;
if (event == null) event = nil;
      event.$prevent();
        ...
        list.$append(item);
        return (($a = [""]), $c = text_box, $c['$value='].apply($c, $a), $a[$a.length-1]);}, TMP_2.$$s = self, TMP_2), $a).call($c, "submit");
    }, nil) && 'initialize'
  })(self, null)
};
...
(function(Opal) {
  Opal.dynamic_require_severity = "error";
  var $a, $b, TMP_2, self = Opal.top, $scope = Opal, nil = Opal.nil, $breaker = Opal.breaker, $slice = Opal.slice, $klass = Opal.klass, $hash2 = Opal.hash2;

  Opal.add_stubs(['$require', '$find', '$new', '$add_class', '$html=', '$on', '$prevent', '$animate', '$append', '$ready?']);
  self.$require("opal");
  self.$require("opal-jquery");
  self.$require("application"+ '/../' + "application-todo.rb");
  (function($base, $super) {
    function $Content(){};
    var self = $Content = $klass($base, $super, 'Content', $Content);

    var def = self.$$proto, $scope = self.$$scope;

    return (def.$initialize = function() {
      var $a, $b, TMP_1, self = this, content = nil, active_dot = nil;

      content = $scope.get('Element').$find("#content");
      active_dot = $scope.get('Element').$new("div");
      active_dot.$add_class("dot");
      (($a = ["Tap"]), $b = active_dot, $b['$html='].apply($b, $a), $a[$a.length-1]);
      ($a = ($b = active_dot).$on, $a.$$p = (TMP_1 = function(event){var self = TMP_1.$$s || this;
if (event == null) event = nil;
      event.$prevent();
        return $scope.get('Element').$find(".dot").$animate($hash2(["right"], {"right": "80%"}));}, TMP_1.$$s = self, TMP_1), $a).call($b, "click");
      return content.$append(active_dot);
    }, nil) && 'initialize'
  })(self, null);
  return ($a = ($b = $scope.get('Document'))['$ready?'], $a.$$p = (TMP_2 = function(){var self = TMP_2.$$s || this;

  $scope.get('Content').$new();
    return $scope.get('TODO').$new();}, TMP_2.$$s = self, TMP_2), $a).call($b);
})(Opal);
