...
Opal.modules["application-todo"] = function(Opal) {
  ...
  Opal.add_stubs(['$find', '$on', '$prevent', '$new', '$html=', '$value', '$remove', '$append', '$value=']);
  return (function($base, $super) {
    function $TODO(){};
    ...
    return (def.$initialize = function() {
      var $a, $b, TMP_1, self = this, list = nil, form = nil, text_box = nil;

      list = $scope.get('Document').$find("#list");
      form = $scope.get('Document').$find("#form");
      text_box = form.$find("input");
      return ($a = ($b = form).$on, $a.$$p = (TMP_1 = function(event){var self = TMP_1.$$s || this, $a, $b, TMP_2, $c, item = nil;
if (event == null) event = nil;
      event.$prevent();
        item = $scope.get('Element').$new("div.row");
        (($a = ["<div class='card'><p>" + (text_box.$value()) + "</p></div>"]), $b = item, $b['$html='].apply($b, $a), $a[$a.length-1]);
        ($a = ($b = item).$on, $a.$$p = (TMP_2 = function(){var self = TMP_2.$$s || this;

        return item.$remove()}, TMP_2.$$s = self, TMP_2), $a).call($b, "click");
        list.$append(item);
        return (($a = [""]), $c = text_box, $c['$value='].apply($c, $a), $a[$a.length-1]);}, TMP_1.$$s = self, TMP_1), $a).call($b, "submit");
    }, nil) && 'initialize'
  })(self, null)
};
